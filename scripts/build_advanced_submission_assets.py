...
REFERENCES = [
    ...
    f"tb-progression-transcriptome-meta repository. Available from: {REPO_URL}.",
]
...
def cohort_summary() -> pd.DataFrame:
    ...
    for cohort_id in ["GSE107994", "GSE193777"]:
        ...
        age = pd.to_numeric(meta.get("age_years"), errors="coerce") if "age_years" in meta.columns else pd.Series(dtype=float)
        sex = meta.get("sex", pd.Series(dtype=object)).astype(str).str.upper()
        rows.append(
            {
                "Cohort": cohort_id,
...
def manuscript_text(data: AdvancedData) -> dict[str, list[str] | str]:
    ...
    intro = [
        ...
        "The first manuscript from this repository addressed reproducible signature discovery and cohort-held-out validation. The present study asks a different question. Instead of focusing primarily on classifier performance, it evaluates whether the same public datasets support deeper biological structure through unsupervised analysis, Bayesian shrinkage, pathway-level synthesis, and a causal-interpretation framework.",
    ...
    methods = [
        ...
        "To extend biological interpretation, we also applied two OmicsClaw-inspired bulk-RNA analyses. First, a conservative non-negative least squares deconvolution used canonical blood-cell marker sets to estimate relative composition proxies across samples. These outputs were interpreted as marker-based composition scores rather than absolute leukocyte fractions. Second, a WGCNA-style coexpression analysis of the most variable shared genes was used to derive module eigengenes and hub-gene summaries. These modules were treated as supportive program-level structure and not as causal network estimates.",
    ...
    results = [
        ...
        (f"OmicsClaw-inspired extension analyses supported the same biological direction. Marker-based NNLS deconvolution showed higher {top_cell['cell_type'].lower()}-associated scores in progressors (delta {top_cell['delta_progressor_minus_nonprogressor']:.3f}, p={top_cell['ttest_pvalue']:.2e}), alongside lower T-cell, B-cell, and NK-cell associated scores. In parallel, coexpression analysis identified a large progressor-associated module, {top_module['module']}, containing {int(top_module['n_genes'])} genes with strong eigengene separation by status (p={top_module['ttest_pvalue']:.2e})." + (f" This module overlapped the top Bayesian signal through {top_overlap['signature_genes'].replace('; ', ', ')}." if top_overlap is not None and isinstance(top_overlap['signature_genes'], str) and top_overlap['signature_genes'] else "") if top_cell is not None and top_module is not None else ""),
    ]
...
def build_manuscript(data: AdvancedData) -> Path:
    ...
    factor_table = data.factors.copy().round({"mean_progressor": 3, "mean_nonprogressor": 3, "ttest_pvalue": 6})
    ...
    add_table(doc, data.cohort_summary, "Table 1. Cohort summary for the advanced shared-gene analysis.")
    add_table(doc, top_genes_table, "Table 2. Top Bayesian gene-level posterior effects.")
    add_table(doc, factor_table, "Table 3. Latent factor differences between progressors and non-progressors.")
    add_table(doc, pathway_table, "Table 4. Leading pathway-level Bayesian posterior effects.")
...
def build_review_note() -> Path:
    lines = [
        ...
        "- Confirmed that cohort counts, progressor counts, posterior means, factor p values, and top pathway terms match generated outputs.",
