...
def manuscript_text(data: AdvancedData) -> dict[str, list[str] | str]:
    ...
    discussion = [
        ...
        "Even with those constraints, the results are useful. They identify genes that remain strong after Bayesian shrinkage, show that progression-associated structure survives cross-platform harmonization, and provide a more mechanistic bridge between ranked biomarkers and coordinated host programs. The next technical priority should be to bring additional cohorts into a comparable gene-symbol framework so that the posterior summaries can be tested across a broader evidence base and closer to clinically relevant prediction windows.",
    ]
