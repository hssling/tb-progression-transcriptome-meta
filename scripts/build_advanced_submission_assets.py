...
def manuscript_text(data: AdvancedData) -> dict[str, list[str] | str]:
    ...
    sensitivity_path = ROOT / "results" / "advanced_analysis_gse79362_sensitivity" / "bayesian_gene_meta_with_gse79362.csv"
    sensitivity_bayes = pd.read_csv(sensitivity_path) if sensitivity_path.exists() else pd.DataFrame()
    sensitivity_top = sensitivity_bayes.iloc[0] if not sensitivity_bayes.empty else None
    abstract = [
    ...
    methods = [
        ...
        "Because GSE79362 was available locally as a junction-level RNA-sequencing workbook with an embedded gene column, we also performed an exploratory remapping step in which junction counts were aggregated to gene symbols. To avoid pseudo-replication from repeated follow-up samples, a one-subject-one-sample earliest-available rule was applied for sensitivity analysis. This remapped cohort was then used only for exploratory triangulation and not for replacing the primary two-cohort estimates.",
    ...
    results = [
        ...
        (f"Additional deconvolution and coexpression analyses supported the same biological direction. Marker-based NNLS deconvolution showed higher {top_cell['cell_type'].lower()}-associated scores in progressors (delta {top_cell['delta_progressor_minus_nonprogressor']:.3f}, p={top_cell['ttest_pvalue']:.2e}, FDR={top_cell['fdr_bh']:.2e}), alongside lower T-cell, B-cell, and NK-cell associated scores. Platelet- and neutrophil-associated scores also shifted upward, although less strongly than the monocyte-associated component. In parallel, coexpression analysis identified a large progressor-associated module, {top_module['module']}, containing {int(top_module['n_genes'])} genes with strong eigengene separation by status (p={top_module['ttest_pvalue']:.2e}, FDR={top_module['fdr_bh']:.2e})." + (f" This module overlapped the top Bayesian signal through {top_overlap['signature_genes'].replace('; ', ', ')}." if top_overlap is not None and isinstance(top_overlap['signature_genes'], str) and top_overlap['signature_genes'] else "") + " Taken together, these extension layers suggest that the transcriptomic progression signal is shaped by both coordinated cell-state biology and shifts in relative blood-cell representation." if top_cell is not None and top_module is not None else ""),
        (f"Exploratory expansion of the evidence base showed that GSE79362 can be brought into the shared-gene space, but only with additional caution. Junction-to-gene remapping produced 10,419 genes overlapping the two-cohort shared space and allowed construction of a subject-level sensitivity subset with 107 samples, including 33 progressors and 74 non-progressors. When this remapped cohort was added in exploratory Bayesian synthesis, the strongest pooled genes shifted toward {sensitivity_top['gene']}, and other myeloid-associated genes such as FCGR3B, HP, and ACSL1 rose to the top ranks (Table 5)." if sensitivity_top is not None else ""),
        "This sensitivity result is important in itself. It shows that public-data expansion is feasible, but it also shows that broader evidence integration may change the apparent center of gravity of the signature. For that reason, the remapped GSE79362 cohort was treated as sensitivity evidence rather than merged into the primary biological claims.",
    ]

    discussion = [
        "This reanalysis adds scientific meaning in four linked ways. First, it shows that cohort structure materially affects the leading unsupervised axes and must be addressed before biological interpretation is attempted. Second, it replaces simple ranking with posterior estimates and uncertainty intervals. Third, it demonstrates convergence between gene-level, factor-level, pathway-level, deconvolution, and network-level summaries. Fourth, it makes the limits of causal interpretation explicit rather than leaving them implicit.",
        "The leading genes point toward a host program centered on myeloid regulation, cell trafficking, scavenger biology, and compensatory immune control. MILR1, VSIG4, CCR2, CD36, FZD5, and AQP1 do not read like a random collection of high-scoring variables. Together they suggest coordinated changes in cellular recruitment, innate regulation, and tissue interaction, which is more persuasive biologically than a diffuse inflammatory signature.[[2-5,11]]",
        "These findings fit the broader tuberculosis transcriptomic literature, but they are not redundant with it. Zak and colleagues showed that prospective blood signatures can identify short-term risk of disease.[[2]] RISK6 demonstrated that some of that biology can be compressed into a smaller signature with diagnostic and treatment-response relevance.[[11]] Pooled comparisons of concise incipient-TB signatures showed that several models perform similarly and that prognostic performance usually decays as the interval before disease increases.[[12]] More recent validation studies in pragmatic or geographically distinct settings have reinforced how dependent performance is on context, thresholding, and time-to-disease windows.[[13,14]]",
        "The exploratory GSE79362 remap adds a useful counterweight to overconfident interpretation. Once that cohort was aggregated from junctions to genes and reduced to one sample per subject, the strongest pooled genes were no longer identical to those in the primary two-cohort model. That does not invalidate the original findings. Instead, it shows that adding a large prospective RNA-sequencing cohort introduces biologically plausible but non-trivial shifts in the ranking landscape, especially toward genes linked to myeloid activation and acute-phase signaling. This is exactly the kind of heterogeneity that a robust translational biomarker program must be able to confront rather than conceal.",
        "The pathway and coexpression results sharpen biological interpretation beyond what a ranked list can provide. The repeated appearance of angiogenesis and vasculature-development terms should not be read as a claim that the disease process is primarily vascular. A more defensible reading is that progression-risk biology in blood may include immune-cell behavior that tracks with tissue remodeling and evolving host-pathogen interface changes. The large progressor-associated coexpression module is consistent with that view because it suggests that the leading genes sit within broader coordinated programs rather than operating as isolated markers.",
        "The deconvolution layer supports the same direction of interpretation, especially the relative shift toward monocyte-weighted and away-from-lymphoid weighted profiles in progressors. That observation is biologically plausible in light of the Bayesian enrichment of genes such as CD36 and VSIG4, but it still needs to be interpreted carefully. The NNLS deconvolution layer uses canonical marker genes and therefore yields relative composition proxies, not direct leukocyte fractions measured by flow cytometry or single-cell profiling. Its value lies in showing compatibility with compositional shifts, not in providing exact cell counts.",
    ...
    conclusion = (
        ...
        "predictive claims are made. Exploratory remapping of GSE79362 shows that such expansion is feasible but may materially shift the pooled signature, underscoring the need for cautious, design-aware integration. The deconvolution results should be read as marker-based composition proxies, "
...
def build_manuscript(data: AdvancedData) -> Path:
    ...
    sensitivity_table_path = ROOT / "results" / "advanced_analysis_gse79362_sensitivity" / "bayesian_gene_meta_with_gse79362.csv"
    sensitivity_table = pd.read_csv(sensitivity_table_path).head(8).loc[:, ["gene", "posterior_mean", "ci95_low", "ci95_high"]] if sensitivity_table_path.exists() else pd.DataFrame()
    ...
    if not sensitivity_table.empty:
        add_table(doc, sensitivity_table, "Table 5. Top exploratory Bayesian gene-level posterior effects after adding the remapped GSE79362 sensitivity cohort. These results are shown to illustrate evidence expansion and heterogeneity, not to replace the primary two-cohort analysis.")
