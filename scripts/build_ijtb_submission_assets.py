...
def build_main_manuscript(data: ManuscriptData) -> Path:
    ...
    add_cited_paragraph(doc, "The top-ranked genes were biologically coherent rather than statistically isolated. MILR1 and IRAK3 suggest regulatory signaling, CCR2 and ITGB2 support immune-cell recruitment, CD36 reflects macrophage-associated lipid and scavenger biology, and FZD5 points to a broader signaling context that may relate to tissue remodeling and inflammatory balance. The forest summary of top transcriptomic effects is shown in Fig. 1.")
    add_cited_paragraph(doc, "The leading genes also showed a useful balance between effect size and interpretability. For a translational manuscript, it is not enough for genes to rank highly; they should also map to host processes that clinicians and laboratory scientists can understand. In that respect, the current list is more persuasive than a purely opaque machine-learning feature set because it links progression to recognizable pathways in myeloid activation, adhesion, and immune regulation.")
    ...
    add_cited_paragraph(doc, "The results also showed that discrimination and calibration were not equivalent. The gene-set-score model ranked samples best, but linear support vector machine achieved more favorable Brier scores. This pattern suggests that the host signal is strong enough for cross-cohort discrimination, while absolute risk estimation remains less mature. The ROC comparison is presented in Fig. 2.")
    ...
    discussion = [
        ...
        "Even with those limitations, the study remains useful. It identifies a biologically interpretable candidate gene set, demonstrates that cross-cohort signal retention is achievable, and provides a reproducible submission package that can be updated as more progression-ready cohorts are integrated.",
