...
def manuscript_text(data: dict[str, pd.DataFrame]) -> dict[str, list[str] | str]:
    ...
    display_names = {
        "vascular_proxy5": "vascular-remodeling proxy score",
        "remap_myeloid5": "remap-sensitive myeloid score",
        "module_M6_proxy": "M6 coexpression-program score",
        "Neutrophil": "neutrophil proxy score",
        "Platelet": "platelet proxy score",
        "Monocyte": "monocyte proxy score",
        "Tcell": "T-cell proxy score",
        "Bcell": "B-cell proxy score",
        "NK": "NK-cell proxy score",
        "signature25_present": "25-gene signature proxy",
        "bayes_core7": "compact Bayesian core score",
    }

    def label(name: str) -> str:
        return display_names.get(name, name)

    top_programs = [label(x) for x in program_models.head(5)["feature"].tolist()]
    top_genes = gene_models.head(3)["feature"].tolist()
    fdr10_programs = [label(x) for x in program_models.loc[program_models["interaction_fdr_bh"] < 0.10, "feature"].tolist()]
    ...
    abstract = [
        "Background: Most blood transcriptomic studies of tuberculosis progression summarize baseline discrimination, but prospective cohorts with repeated samples can also clarify whether host-response programs change differently over follow-up in progressors and non-progressors. We analyzed a prospective public RNA-sequencing cohort after gene-level remapping to test that question.",
    ...
    results = [
        ...
        f"Program-level modeling produced a clearer signal than single-gene modeling. The strongest progressor-by-month interactions were observed for {', '.join(top_programs)} (Table 2; Fig. 3). At a false-discovery threshold below 0.10, the retained program-level features were {', '.join(fdr10_programs)}. The negative interaction coefficients for the vascular-remodeling proxy score, remap-sensitive myeloid score, and M6 coexpression-program score indicate steeper decline over follow-up in progressors than in non-progressors, whereas the platelet proxy score showed a modest positive interaction.",
        f"Descriptive mean trajectories supported the same interpretation. In progressors, the remap-sensitive myeloid score rose between OTHER or DAY0 and DAY180, then decreased by DAY540, while the vascular-remodeling proxy score also shifted from early positive values toward negative values later in follow-up (Fig. 2). Non-progressors showed flatter or smaller changes over the same scheduled visits. For example, the progressor mean for the remap-sensitive myeloid score moved from {float(day180_prog['remap_myeloid5'].iloc[0]):.3f} at DAY180 to {float(day540_prog['remap_myeloid5'].iloc[0]):.3f} at DAY540, whereas the non-progressor means remained closer to {float(day180_non['remap_myeloid5'].iloc[0]):.3f} and {float(day540_non['remap_myeloid5'].iloc[0]):.3f}, respectively.",
        ...
        "Taken together, the results support three linked inferences. First, longitudinal differences exist within this remapped cohort. Second, those differences are better captured by grouped biology than by single genes. Third, the strongest longitudinal story involves myeloid-weighted and vascular-linked programs that change over follow-up rather than remaining fixed at one elevated level.",
    ]

    discussion = [
        "This analysis suggests that longitudinal progression biology in this cohort is easier to detect at the level of coordinated host programs than at the level of individual genes. That is an important distinction. It implies that repeated measurements may capture dynamic reweighting of related pathways, cell states, and modules even when no one transcript provides a universally stable trajectory marker.",
...
def build_highlights() -> Path:
    ...
    for line in [
        "Longitudinal analysis was performed on a remapped prospective TB RNA-sequencing cohort with repeated subject-level follow-up.",
